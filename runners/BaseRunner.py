        ...
        try:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    self.global_step += 1
                    # self.net.train()
                    self.net.denoise_fn.train()
                    if hasattr(self.net, "noise_approximator_fn") and self.net.noise_approximator_fn is not None:
                        self.net.noise_approximator_fn.train()

                    losses = []
                    for i in range(len(self.optimizer)):

                        # pdb.set_trace()
                        loss = self.loss_fn(net=self.net, batch=train_batch, epoch=epoch, step=self.global_step, opt_idx=i, stage='train')
                        loss.backward()
