...
from model.BrownianBridge.DualBrownianBridgeModel import DualBrownianBridgeModel
from model.BrownianBridge.ABridge import ABridgeModel
...
from model.BrownianBridge.LinearABridge import LinearABridgeModel
from model.BrownianBridge.LinearABridge_without_t import LinearABridgeModelWt
...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_model(self, config):
        ...
        elif config.model.model_type == "DBBDM":
            bbdmnet = DualBrownianBridgeModel(config.model).to(config.training.device[0])
        elif config.model.model_type == "ABridge":
            bbdmnet = ABridgeModel(config.model).to(config.training.device[0])
        elif config.model.model_type == "LinearABridge":
            bbdmnet = LinearABridgeModel(config.model).to(config.training.device[0])
        elif config.model.model_type == "LinearABridge_without_condition":
            bbdmnet = LinearABridgeModelWt(config.model).to(config.training.device[0])
        else:
            raise NotImplementedError
        bbdmnet.apply(weights_init)
        return bbdmnet

    ### TODO: Change the following function to accept DualBrownianBridgeModel
    ...
    def initialize_optimizer_scheduler(self, net, config):
        if config.model.model_type == "DBBDM":
            approximator1, approximator2 = net.get_parameters() # approximator1 is denoise_fn, approximator2 is noise_approximator_fn
            optimizer1 = get_optimizer(config.model.BB.optimizer, approximator1)
            scheduler1 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer1,
                                                                   mode='min',
                                                                   verbose=True,
                                                                   threshold_mode='rel',
                                                                   **vars(config.model.BB.lr_scheduler)
            )
            optimizer2 = get_optimizer(config.model.BB.optimizer, approximator2)
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        if opt_idx == 1: # load from cache
            return BBDMRunner.noise_loss
        (x, x_name), (x_cond, x_cond_name) = batch
        x = x.to(self.config.training.device[0])
        x_cond = x_cond.to(self.config.training.device[0])

        losses, additional_info = net(x, x_cond)
        if isinstance(losses, list):
            loss = losses[0]
            # TODO this needs to be modified with better design
            BBDMRunner.noise_loss = losses[1]
            BBDMRunner.kl_loss = losses[2]
        else:
            loss = losses
        if write and self.is_main_process:
            ...
            if additional_info.__contains__('kl_loss'):
                self.writer.add_scalar(f'kl_loss/{stage}', additional_info['kl_loss'], step)
            if additional_info.__contains__('recloss'):
                self.writer.add_scalar(f'recloss/{stage}', additional_info['recloss'], step)
        return loss
