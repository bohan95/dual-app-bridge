...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        # weather need kl loss
        self.use_kl = model_params.use_kl
        if self.use_kl == True:
            print("use kL")
        else:
            print("Don't kL")
    ...
    def p_losses(self, x0, y, context, t, noise=None):
        ...
        # TODO This is testing code, need to know if we needed
        if self.use_kl:
            mu = objective_recon.mean(dim=(1, 2, 3))  # (B,)
            sigma = objective_recon.std(dim=(1, 2, 3))  # (B,)

            kl = 0.5 * (mu**2 + sigma**2 - torch.log(sigma**2 + 1e-8) - 1)  # (B,)
            kl_loss = kl.mean()
                
            lambda_kl = 1e-3  # 你可以调这个超参数
            total_loss = recloss + lambda_kl * kl_loss
            log_dict = {
                "loss": total_loss,
                "recloss": recloss,
                'kl_loss': kl_loss
